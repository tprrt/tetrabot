...
void GL_Piston::keyboardCallback(unsigned char key, int x, int y)
{
	(void)x;
	(void)y;
	...
	case 'z' :{	printf("Indication du piston 0\n"); 
	// placer la boule 4 au centre de gravite
	const btVector3& b0 =  robot->Sommets[0]->getBody()->getCenterOfMassPosition();
	const btVector3& b1 =  robot->Sommets[1]->getBody()->getCenterOfMassPosition();
	const btVector3& b2 =  robot->Sommets[2]->getBody()->getCenterOfMassPosition();
	const btVector3& b3 =  robot->Sommets[3]->getBody()->getCenterOfMassPosition();
...
void GL_Piston::specialKeyboard(int key, int x, int y)	
{
	...
	{
		printf("F1\n");
		Thread(this,GL_Piston::marcherRobot);
		break;
	}
...
bool GL_Piston::IsNotInArea(const btVector3 &G,const btVector3 &end2){
	return (
...
void* GL_Piston::marcherRobot(void *demo)
{
	...
	while(This->IsNotInArea(This->robot->getCenterOfMassPosition(),end))
	{
		...
		//printf("P1: %d, P2: %d\n",P1,P2);
		This->robot->Arcs[P1]->debloquerPiston();
		This->robot->Arcs[P2]->debloquerPiston();
		...
		This->robot->Arcs[P1]->bloquerPiston();
		This->robot->Arcs[P2]->bloquerPiston();
