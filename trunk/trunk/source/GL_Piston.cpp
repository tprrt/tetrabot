...
*/
	...
	btBoxShape* shapeCube = new btBoxShape(btVector3(btScalar(5.),btScalar(0.1),btScalar(5.)));
	...
	// Creation du robot
	this->robot = new Robot_Tetra(this->m_dynamicsWorld,btVector3(btScalar(0.),btScalar(10.),btScalar(0.)));
...
void GL_Piston::specialKeyboard(int key, int x, int y)	
{
	...
	case GLUT_KEY_F6:
		...
		case GLUT_KEY_F8:
		{
			...
			AlgoGen_Tetra* algo = new AlgoGen_Tetra(this->m_dynamicsWorld,this->robot,btVector3(0,0,0),btScalar(0.05),2,4,3,2);
			algo->run();
			break;
		}
	case GLUT_KEY_END:
...
void* GL_Piston::marcherRobot(void *demo)
{
	...
	// On choisi un point d'arrive du robot sur le plan XZ
	...
	// est inferieure à 6.
	while( (end.distance(This->robot->getCenterOfMassPosition())) > btScalar(6.))
	{
