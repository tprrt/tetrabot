...
Noeud::Noeud(btDynamicsWorld *ownerWorld,Ogre::SceneNode * nodeOgre,btConvexInternalShape* forme,const btVector3 & offset,btScalar masse)
{
	...
	// Friction boule/sol
	this->bodyNoeud->setFriction(FRICTION);
}

Noeud::Noeud(btDynamicsWorld *ownerWorld,btConvexInternalShape* forme,const btVector3 & offset, btScalar masse)
{
	static int ID=0;
	this->id = ID;
	ID++;
...
btVector3 Noeud::getCenterOfMassPosition()
{
	return this->bodyNoeud->getCenterOfMassPosition();
}


btRigidBody* Noeud::getBody()
{
	return this->bodyNoeud;
}
void Noeud::setBody(btRigidBody *body)
{
	this->bodyNoeud = body;
}

btConvexInternalShape* Noeud::getShapeNoeud()
{
	return this->shapeNoeud;
}
void Noeud::setShapeNoeud(btConvexInternalShape* shape)
{
	this->shapeNoeud = shape;
}


int Noeud::lierPiston(Piston *piston, char extremitee, btScalar rotationY, btScalar rotationZ)
{
	btRigidBody* bodyBox;
	btBoxShape* boxShape;
	...
	*/
	...
	coneC->setLimit(ANGLES_CONE,ANGLES_CONE,btScalar(0.));
